...
async def liveness_check() -> Response:
    ...
    """
    return Response(
        content='{"status": "alive"}', media_type=CONTENT_TYPE_APPLICATION_JSON, status_code=HTTP_OK
    )
...
async def startup_check() -> Response:
    ...
    return Response(
        content='{"status": "started"}', media_type=CONTENT_TYPE_APPLICATION_JSON, status_code=HTTP_OK
    )
