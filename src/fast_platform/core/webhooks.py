...
from .constants import CONTENT_TYPE_APPLICATION_JSON, DEFAULT_TIMEOUT_SECONDS, HEADER_AUTHORIZATION, HEADER_CONTENT_TYPE, HEADER_X_WEBHOOK_SIGNATURE
...
from fast_platform.core.registry import registry
...
class WebhookManager:
    ...
    async def _deliver(
        ...
        if subscription.auth_type == WebhookAuthType.HMAC:
            ...
            headers[HEADER_X_WEBHOOK_SIGNATURE] = f"sha256={signature}"
        elif (
            subscription.auth_type == WebhookAuthType.BEARER and subscription.auth_token
        ):
            headers[HEADER_AUTHORIZATION] = f"Bearer {subscription.auth_token}"
        elif (
