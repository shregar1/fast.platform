...
class HealthEndpoint:
    ...
    def create_router(self) -> APIRouter:
        ...
        async def health() -> Response:
            ...
            status_code = (
                status.HTTP_200_OK if overall == HealthStatus.HEALTHY
        ...
        async def ready() -> Response:
            ...
            body = self._format_response(results, 
                HealthStatus.HEALTHY if is_ready else HealthStatus.UNHEALTHY)
            return Response(
                content=body,
                status_code=status_code,
                media_type="application/json"
        ...
        async def live() -> Response:
            ...
            status_code = (
                status.HTTP_200_OK if is_alive
                else status.HTTP_503_SERVICE_UNAVAILABLE
            )
        ...
        async def startup() -> Response:
            ...
            status_code = (
                status.HTTP_200_OK if overall != HealthStatus.UNHEALTHY
                else status.HTTP_503_SERVICE_UNAVAILABLE
            )
