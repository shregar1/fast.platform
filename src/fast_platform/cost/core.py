...
class CloudProvider(Enum):
    """Represents the CloudProvider class."""
...
current_cost_context: ContextVar[Optional["CostContext"]] = ContextVar("cost_context", default=None)
...
class CostContext:
    ...
    def record(
        self, resource_type: ResourceType, quantity: Decimal, unit: str, resource_id: str, **tags
    ) -> CostEvent:
        """Record a cost event."""
        from uuid import uuid4
...
class CostTracker:
    ...
    @staticmethod
    def record(
        resource_type: ResourceType, quantity: Decimal, unit: str, resource_id: str, **tags
    ) -> Optional[CostEvent]:
