...
class BudgetAlert:
    ...
    @classmethod
    def at(cls, percent: float, notify: str = "slack", action: str = "notify") -> "BudgetAlert":
        """Convenience factory."""
        return cls(threshold_percent=percent, action=action, channels=[notify])
...
class BudgetManager:
    ...
    @classmethod
    async def update_spend(cls, tenant_id: Optional[str], amount: Decimal) -> None:
    ...
    @classmethod
    def _get_budget_for_tenant(cls, tenant_id: Optional[str]) -> Optional[Budget]:
