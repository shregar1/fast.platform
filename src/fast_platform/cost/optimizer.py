...
class CostOptimizer:
    ...
    async def _analyze_compute(
        ...
        for service, utilization in metrics.items():
            if utilization < 0.2:  # < 20% utilization
                current_cost = await self._get_service_cost(
                    service, ResourceType.COMPUTE
                )
    ...
    async def _analyze_storage(
        ...
        old_data = await DataAnalyzer.find_old_data(
