...
class ShutdownManager:
    ...
    def register(
        ...
        hook = ShutdownHook(
            name=name or handler.__name__, handler=handler, priority=priority, timeout=timeout
        )
...
def on_shutdown(name: Optional[str] = None, priority: int = 0, timeout: float = 30.0):
...
def on_startup(name: Optional[str] = None, priority: int = 0, timeout: float = 30.0):
