...
class S3Client:
    ...
    async def upload_file(
        ...
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(
                pool, lambda: client.upload_fileobj(file, bucket_name, key, ExtraArgs=extra_args)
            )
    ...
    async def download_file(self, key: str, bucket: Optional[str] = None) -> bytes:
        ...
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(
                pool, lambda: client.download_fileobj(bucket_name, key, buffer)
            )

        return buffer.getvalue()

    async def delete_file(self, key: str, bucket: Optional[str] = None) -> bool:
    ...
    async def generate_presigned_url(
        ...
        url = client.generate_presigned_url(
            ClientMethod=operation, Params={"Bucket": bucket_name, "Key": key}, ExpiresIn=expiration
        )

        from datetime import timedelta

        return PresignedUrl(url=url, expires_at=datetime.utcnow() + timedelta(seconds=expiration))
