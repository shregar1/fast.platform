...
class SESClient:
    ...
    def __init__(
        ...
        Args:
            ...
            configuration_set: The configuration_set parameter.
        """
        self.region = region
    ...
    async def send_email(
        ...
        params = {
            ...
            "Message": {"Subject": {"Data": subject}, "Body": message},
        }
        ...
        with ThreadPoolExecutor() as pool:
            response = await loop.run_in_executor(pool, lambda: client.send_email(**params))

        return response["MessageId"]

    async def send_templated_email(
        self, source: str, template: str, template_data: Dict[str, Any], to_addresses: List[str]
    ) -> str:
        ...
        with ThreadPoolExecutor() as pool:
            response = await loop.run_in_executor(
                pool, lambda: client.send_templated_email(**params)
            )
    ...
    async def send_bulk_templated_email(
        ...
        params = {"Source": source, "Template": template, "Destinations": destinations}
        ...
        with ThreadPoolExecutor() as pool:
            response = await loop.run_in_executor(
                pool, lambda: client.send_bulk_templated_email(**params)
            )
    ...
    async def create_template(
        ...
        template = {"TemplateName": name, "SubjectPart": subject}
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(pool, lambda: client.create_template(Template=template))
    ...
    async def verify_email_identity(self, email: str) -> bool:
        ...
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(
                pool, lambda: client.verify_email_identity(EmailAddress=email)
            )
    ...
    async def get_send_quota(self) -> Dict[str, Any]:
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            response = await loop.run_in_executor(pool, client.get_send_quota)
    ...
    async def get_send_statistics(self) -> List[Dict[str, Any]]:
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            response = await loop.run_in_executor(pool, client.get_send_statistics)
