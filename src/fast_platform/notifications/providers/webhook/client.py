...
class WebhookClient:
    ...
    def __init__(
        ...
        Args:
            ...
            signature_header: The signature_header parameter.
        """
        self.url = url
    ...
    async def send(
        self, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None, sign: bool = True
    ) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                self.url, data=payload_json, headers=request_headers
            ) as response:
                return {
    ...
    def verify_incoming_request(
        payload: bytes, signature_header: str, secret: str, scheme: str = "hmac-sha256"
    ) -> bool:
        ...
        return WebhookSignature.verify_signature(payload.decode(), signature_header, secret)
