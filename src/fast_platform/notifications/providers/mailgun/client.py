    ...
    async def send_email(
        ...
        if attachments:
            for att in attachments:
                files.append(
                    ("attachment", (att.filename, att.content, att.content_type or "application/octet-stream"))
                )
        ...
        async with aiohttp.ClientSession() as session:
            if files:
                ...
                for name, file_info in files:
                    data_sent.add_field(name, file_info[1], filename=file_info[0], content_type=file_info[2])
