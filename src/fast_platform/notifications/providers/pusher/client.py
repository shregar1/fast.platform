...
class PusherClient:
    ...
    def __init__(self, app_id: str, key: str, secret: str, cluster: str = "mt1", ssl: bool = True):
    ...
    async def trigger(
        self, channels: List[str], event: str, data: Dict[str, Any], socket_id: Optional[str] = None
    ) -> bool:
        ...
        kwargs = {"channels": channels, "event_name": event, "data": data}
        ...
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(pool, lambda: client.trigger(**kwargs))
    ...
    async def trigger_batch(self, events: List[Dict[str, Any]]) -> bool:
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(pool, lambda: client.trigger_batch(batch=events))

        return True

    async def authenticate_private_channel(
        self, socket_id: str, channel_name: str
    ) -> Dict[str, str]:
        ...
        with ThreadPoolExecutor() as pool:
            auth = await loop.run_in_executor(
                pool, lambda: client.authenticate(channel=channel_name, socket_id=socket_id)
            )
        ...
        with ThreadPoolExecutor() as pool:
            auth = await loop.run_in_executor(
                ...
                lambda: client.authenticate(
                    channel=channel_name, socket_id=socket_id, custom_data=auth_data
                ),
            )
    ...
    async def get_channel_info(
        self, channel_name: str, info_attributes: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor() as pool:
            info = await loop.run_in_executor(
                pool, lambda: client.channel_info(channel_name, params)
            )

        return info

    async def get_channels(
        self, prefix_filter: Optional[str] = None, info_attributes: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            channels = await loop.run_in_executor(pool, lambda: client.channels_info(params))
    ...
    async def get_users_in_channel(self, channel_name: str) -> List[Dict[str, Any]]:
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            users = await loop.run_in_executor(pool, lambda: client.users_info(channel_name))

        return users.get("users", [])

    def webhook_handler(self, webhook_secret: str) -> Callable:
