...
class EventStream:
    ...
    def __init__(self, heartbeat_interval: float = 30.0, retry_timeout: int = 3000):
        """Execute __init__ operation.

        Args:
            heartbeat_interval: The heartbeat_interval parameter.
            retry_timeout: The retry_timeout parameter.
        """
    ...
    async def send(self, data: Any, event: Optional[str] = None, id: Optional[str] = None) -> None:
        """Send an event."""
        if isinstance(data, dict):
            data = json.dumps(data)

        sse_event = SSEEvent(data=str(data), event=event, id=id)
    ...
    async def __aiter__(self) -> AsyncIterator[str]:
        ...
        while not self._closed:
            try:
                event = await asyncio.wait_for(self._queue.get(), timeout=self.heartbeat_interval)
...
def sse_endpoint(path: str, headers: Optional[Dict[str, str]] = None, ping_interval: float = 30.0):
