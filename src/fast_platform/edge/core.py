...
class EdgeFunction:
    ...
    async def invoke(self, request: EdgeRequest) -> EdgeResponse:
        ...
        try:
            ...
            response.headers["x-edge-processing-time"] = str(
            ...
            return EdgeResponse.json(
                {"error": "Edge function timeout", "function": self.name},
                status=504
            )
        except Exception as e:
            return EdgeResponse.json(
                {"error": str(e), "edge_function": self.name},
                status=500
            )
