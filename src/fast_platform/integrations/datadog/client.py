...
class DatadogClient:
    ...
    def __init__(self, api_key: str, app_key: Optional[str] = None, site: Optional[str] = None):
        ...
        Args:
            ...
            app_key: The app_key parameter.
            site: The site parameter.
        """
        self.config = DatadogConfig(api_key=api_key, app_key=app_key, site=site or "datadoghq.com")
        self._api = None

    def _get_api(self):
        """Execute _get_api operation.
        ...
        if self._api is None:
            try:
                ...
                options = {"api_key": self.config.api_key, "app_key": self.config.app_key}
        ...
        with ThreadPoolExecutor() as pool:
            result = await loop.run_in_executor(
                ...
                lambda: api.Event.create(
                    title=title, text=text, alert_type=alert_type, tags=tags or []
                ),
            )
    ...
    async def send_log(
        ...
        headers = {"Content-Type": "application/json", "DD-API-KEY": self.config.api_key}
