...
class ServiceBusClient:
    ...
    def __init__(
        ...
        Args:
            ...
            fully_qualified_namespace: The fully_qualified_namespace parameter.
            credential: The credential parameter.
        """
    ...
    async def send_message(
        ...
        with ThreadPoolExecutor() as pool:
            sender = await loop.run_in_executor(
                pool, lambda: client.get_queue_sender(queue_name=queue_or_topic)
            )
    ...
    async def send_messages_batch(
        self, queue_or_topic: str, messages: List[ServiceBusMessage]
    ) -> None:
            ...
            if topic:
                receiver = await loop.run_in_executor(
                    ...
                    lambda: client.get_subscription_receiver(
                        topic_name=topic, subscription_name=queue_or_subscription
                    ),
                )
            else:
                receiver = await loop.run_in_executor(
                    pool, lambda: client.get_queue_receiver(queue_name=queue_or_subscription)
                )
    ...
    async def complete_message(
        self, queue_or_subscription: str, message: ServiceBusMessage, topic: Optional[str] = None
    ) -> None:
        """Complete a message."""
            ...
            try:
                sequence_number = await loop.run_in_executor(
                    ...
                    lambda: sender.schedule_messages(
                        messages=[message], scheduled_enqueue_time_utc=scheduled_enqueue_time_utc
                    ),
                )
