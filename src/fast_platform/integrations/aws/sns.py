...
class SNSClient:
    ...
    async def unsubscribe(self, subscription_arn: str) -> bool:
        ...
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(
                pool,
                lambda: client.unsubscribe(SubscriptionArn=subscription_arn)
            )
    ...
    async def list_subscriptions(
        ...
        with ThreadPoolExecutor() as pool:
            if topic_arn:
                response = await loop.run_in_executor(
                    pool,
                    lambda: client.list_subscriptions_by_topic(TopicArn=topic_arn)
                )
            else:
                response = await loop.run_in_executor(
                    pool,
    ...
    async def create_topic(self, name: str) -> str:
        ...
        with ThreadPoolExecutor() as pool:
            response = await loop.run_in_executor(
                pool,
                lambda: client.create_topic(Name=name)
            )
    ...
    async def delete_topic(self, topic_arn: Optional[str] = None) -> bool:
        ...
        with ThreadPoolExecutor() as pool:
            await loop.run_in_executor(
                pool,
                lambda: client.delete_topic(TopicArn=arn)
            )
