...
class ILLM(ABC):
    """Marker base for concrete types in the ``llm`` package."""
...
def build_llm_service() -> Optional[ILLMService]:
    ...
    _log = logger if logger is not None else None
    ...
    if cfg.openai.enabled and cfg.openai.api_key:
        try:
            ...
            if _log:
                _log.warning("Failed to initialize OpenAI LLM service: %s", exc)
    ...
    if cfg.anthropic.enabled and cfg.anthropic.api_key:
        try:
            ...
            if _log:
                _log.warning("Failed to initialize Anthropic LLM service: %s", exc)
    ...
    if cfg.ollama.enabled:
        try:
            ...
            if _log:
                _log.warning("Failed to initialize Ollama LLM service: %s", exc)

    if _log:
        _log.info("No LLM provider is enabled.")
    return None
