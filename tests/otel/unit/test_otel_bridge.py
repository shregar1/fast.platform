...
def test_apply_no_trace_module():
    with patch("otel.bridge._trace_api", return_value=None):
...
def test_apply_no_request_id():
    ...
    with patch("otel.bridge._trace_api", return_value=trace), patch(
        "otel.bridge.get_request_id", return_value=None
    ):
        assert apply_request_id_to_current_span() is False
...
def test_apply_sets_attribute():
    ...
    with patch("otel.bridge._trace_api", return_value=trace), patch(
        "otel.bridge.get_request_id", return_value="req-1"
    ):
...
def test_inject_without_otel():
    ...
    with patch("otel.bridge.optional_import", return_value=(None, None)):
...
def test_apply_span_not_recording():
    ...
    trace.get_current_span.return_value = span
    with patch("otel.bridge._trace_api", return_value=trace), patch(
        "otel.bridge.get_request_id", return_value="r"
    ):
        assert apply_request_id_to_current_span() is False
...
def test_inject_success():
    ...
    with patch("otel.bridge.optional_import", side_effect=oi_side_effect):
