...
class TestEnqueue(IJobTests):
    ...
    def _cfg(**overrides: object) -> JobsConfigurationDTO:
        raw = {
            ...
            "rq": {"enabled": False, "redis_url": "redis://localhost:6379/0", "queue_name": "default"},
    ...
    def test_enqueue_celery_apply_async(self, mock_jobs: MagicMock) -> None:
        cfg = self._cfg(celery={"enabled": True, "broker_url": "redis://x", "result_backend": "redis://x"})
    ...
    @patch('rq.Queue')
    @patch('redis.Redis')
    def test_enqueue_rq_enqueue_call(self, _mock_redis: MagicMock, mock_queue_cls: MagicMock, mock_jobs: MagicMock) -> None:
        pytest.importorskip('redis')
        pytest.importorskip('rq')
        cfg = self._cfg(rq={"enabled": True, "redis_url": "redis://localhost:9/0", "queue_name": "default"})
        mock_jobs.return_value.get_config.return_value = cfg
    ...
    def test_enqueue_dramatiq_send_with_options(self, mock_jobs: MagicMock) -> None:
        ...
        cfg = self._cfg(dramatiq={"enabled": True, "broker_url": "redis://x", "queue_name": "default"})
        mock_jobs.return_value.get_config.return_value = cfg
        actor = MagicMock()
