...
class TestInit(IWebhookTests):
    ...
    def test_imports(self):
        from webhooks import compute_signature, verify_signature, deliver_webhook, RetryPolicy, require_webhook_signature
    ...
    async def test_deliver_webhook_success_with_signature_and_non_retryable_error(self):
        ...
        with patch('webhooks.delivery.httpx.AsyncClient') as mock_client:
            ...
            code, err = await deliver_webhook('http://example/hook', b'{}', retry_policy=RetryPolicy(max_attempts=1))
            assert code == 400
