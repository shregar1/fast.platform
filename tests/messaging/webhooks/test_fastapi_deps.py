...
class TestFastapiDeps(IWebhookTests):
    ...
    def test_dependency_rejects_bad_signature(self, app_and_client):
        _, _, client = app_and_client
        payload = b'{}'
        r = client.post('/hook', content=payload, headers={'X-Webhook-Signature': 'sha256=deadbeef'})
        assert r.status_code == 401
    ...
    def test_secret_async_callable(self):
        ...
        @app.post('/h2')
        async def inbound2(raw: Annotated[bytes, Depends(require_webhook_signature(secret=get_secret))]):
            return {'n': len(raw)}
