...
class TestDelivery(IWebhookTests):
    ...
    async def test_deliver_webhook_success(self, monkeypatch):
        ...
        code, err = await deliver_webhook('https://example.com/hook', b'{"ok":true}', retry_policy=RetryPolicy(max_attempts=1))
        assert code == 200
        assert err is None
    ...
    async def test_deliver_webhook_nonretryable_400(self, monkeypatch):
        ...
        cm.__aexit__ = AsyncMock(return_value=None)
        monkeypatch.setattr('webhooks.delivery.httpx.AsyncClient', lambda *a, **k: cm)
        code, err = await deliver_webhook('https://example.com/hook', b'{}', retry_policy=RetryPolicy(max_attempts=1))
        assert code == 400
