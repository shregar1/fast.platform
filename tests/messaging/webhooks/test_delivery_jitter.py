...
class TestDeliveryJitter(IWebhookTests):
    ...
    async def test_jitter_scales_sleep(self, monkeypatch):
        ...
        monkeypatch.setattr("fast_platform.messaging.webhooks.delivery.asyncio.sleep", capture_sleep)
        ...
        with patch("fast_platform.messaging.webhooks.delivery.httpx.AsyncClient", return_value=mock_client):
            with patch("fast_platform.messaging.webhooks.delivery.random.uniform", return_value=1.5):
