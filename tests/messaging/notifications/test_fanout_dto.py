...
import pytest
from notifications.dto import EmailNotificationTarget, NotificationFanoutRequest, NotificationRetryPolicyDTO, PushNotificationTarget
...
class TestFanoutDto(INotificationTests):
    ...
    def test_fanout_request_email_only(self):
        req = NotificationFanoutRequest(title='Hi', body='Body', email=EmailNotificationTarget(to=['a@example.com']))
        assert req.push is None

    def test_fanout_request_push_only(self):
        req = NotificationFanoutRequest(title='Hi', body='Body', push=PushNotificationTarget(ios_device_tokens=['t1']))
        assert req.email is None

    def test_fanout_request_both_channels(self):
        req = NotificationFanoutRequest(title='Hi', body='Body', email=EmailNotificationTarget(to=['a@example.com']), push=PushNotificationTarget(android_registration_tokens=['r1']))
