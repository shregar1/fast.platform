...
from notifications.dto import EmailNotificationTarget, NotificationFanoutRequest, PushNotificationTarget
...
class TestFanoutService(INotificationTests):
    ...
    def test_fanout_dispatch_email_only(self):
        ...
        with patch('notifications.fanout.PushNotificationService') as mock_push_cls:
            ...
            req = NotificationFanoutRequest(title='T', body='B', email=EmailNotificationTarget(to=['u@x.com']))
    ...
    def test_fanout_dispatch_push_only(self):
        ...
        with patch('notifications.fanout.PushNotificationService', return_value=push):
            ...
            req = NotificationFanoutRequest(title='T', body='B', push=PushNotificationTarget(ios_device_tokens=['a'], android_registration_tokens=['b']))
            assert asyncio.run(svc.dispatch(req)) is True
        push.send_to_ios.assert_awaited_once()
        push.send_to_android.assert_awaited_once()

    def test_logging_email_sender(self):
        asyncio.run(LoggingEmailSender().send_email(to=['a@b.com'], subject='S', body_text='text', data={}))
    ...
    def test_fanout_dispatch_fcm_topic_only(self):
        ...
        push.send_to_android = AsyncMock()
        svc = NotificationFanoutService(push=push)
        req = NotificationFanoutRequest(title='T', body='B', push=PushNotificationTarget(fcm_topic='news'))
        assert asyncio.run(svc.dispatch(req)) is True
