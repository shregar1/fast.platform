...
class TestConfigLoader(INotificationTests):
    ...
    def test_push_config_load_from_file(self):
        from notifications.config_loader import PushConfiguration
        PushConfiguration._instance = None
        data = {'apns': {'enabled': True, 'key_id': 'k'}, 'fcm': {'enabled': False}}
        m = mock_open(read_data=json.dumps(data))
        with patch('notifications.config_loader.open', m):
    ...
    def test_push_config_file_not_found(self):
        from notifications.config_loader import PushConfiguration
        PushConfiguration._instance = None
        with patch('notifications.config_loader.open', side_effect=FileNotFoundError()):
            cfg = PushConfiguration()
        dto = cfg.get_config()
    ...
    def test_push_config_json_error(self):
        from notifications.config_loader import PushConfiguration
        PushConfiguration._instance = None
        with patch('notifications.config_loader.open', mock_open(read_data='not-json')):
            with patch('notifications.config_loader.json.load', side_effect=json.JSONDecodeError('bad', 'doc', 0)):
                cfg = PushConfiguration()
        dto = cfg.get_config()
    ...
    def test_push_config_env_overrides(self):
        from notifications.config_loader import PushConfiguration
        PushConfiguration._instance = None
        env = {'APNS_ENABLED': 'true', 'APNS_KEY_ID': 'kid', 'FCM_ENABLED': '1', 'FCM_SERVER_KEY': 'sk'}
        with patch('notifications.config_loader.open', side_effect=FileNotFoundError()):
