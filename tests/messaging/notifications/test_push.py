...
class TestPush(INotificationTests):
    ...
    def test_push_service_apns_disabled(self, mock_cfg_cls):
        ...
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = MagicMock(apns=MagicMock(enabled=False), fcm=MagicMock(enabled=False))
    ...
    def test_push_service_apns_enabled(self, mock_cfg_cls):
        from notifications.push import PushNotificationService
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = MagicMock(apns=MagicMock(enabled=True), fcm=MagicMock(enabled=False))
        mock_cfg_cls.return_value = mock_cfg
        svc = PushNotificationService()
