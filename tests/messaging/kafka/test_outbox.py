...
class TestOutbox(IKafkaTests):
    ...
    def test_publish_outbox_batch(self):
        ...
        async def run():
            batch = [OutboxMessage(id=1, topic='t', value=b'a'), OutboxMessage(id=2, topic='t', value=b'b', headers=[('h', b'v')])]
    ...
    def test_run_outbox_publisher_loop_stops_when_event_set(self):
        ...
        mark = AsyncMock()
        producer = MagicMock()
        asyncio.run(run_outbox_publisher_loop(producer, load, mark, interval_seconds=0.01, stop_event=stop))
    ...
    def test_run_outbox_publisher_loop_processes_one_batch(self):
        ...
        producer = MagicMock()
        producer.send_bytes = AsyncMock()
        asyncio.run(run_outbox_publisher_loop(producer, load, mark_published, limit=10, interval_seconds=0.01, stop_event=stop))
