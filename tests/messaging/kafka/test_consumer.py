...
class TestConsumer(IKafkaTests):
    ...
    def test_consumer_loop_idempotent_dedupes_and_commits(self, mock_cfg_cls):
        from kafka.consumer import KafkaConsumer
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = MagicMock(enabled=True, enable_auto_commit=False, topics=['t1'], bootstrap_servers='localhost:9092', group_id='g')
        mock_cfg_cls.return_value = mock_cfg
        same = b'payload'
    ...
    def test_consumer_loop_idempotent_requires_manual_commit(self, mock_cfg_cls):
        from kafka.consumer import KafkaConsumer
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = MagicMock(enabled=True, enable_auto_commit=True, topics=['t1'], bootstrap_servers='localhost:9092', group_id='g')
        mock_cfg_cls.return_value = mock_cfg
        c = KafkaConsumer()
    ...
    def test_consumer_start_stop_when_enabled(self, mock_cfg_cls, mock_aio_cls):
        from kafka.consumer import KafkaConsumer
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = MagicMock(enabled=True, topics=['t1'], bootstrap_servers='localhost:9092', group_id='g', enable_auto_commit=True)
        mock_cfg_cls.return_value = mock_cfg
        inst = MagicMock()
