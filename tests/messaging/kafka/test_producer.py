...
class TestProducer(IKafkaTests):
    ...
    def test_producer_send_json_envelope_from_dict(self, mock_cfg_cls):
        ...
        p._producer = MM()
        p._producer.send_and_wait = AsyncMock()
        asyncio.run(p.send_json_envelope('events', {'message_type': 'Ping', 'payload': {'ok': True}, 'schema_version': '2'}))
    ...
    def test_producer_send_bytes_with_headers(self, mock_cfg_cls):
        ...
        from kafka.producer import KafkaProducer
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = MM(enabled=True, bootstrap_servers='localhost:9092', dlq_topic=None)
        mock_cfg_cls.return_value = mock_cfg
        p = KafkaProducer()
        p._producer = MM()
        p._producer.send_and_wait = AsyncMock()
    ...
    def test_producer_send_json_envelope_to_dlq(self, mock_cfg_cls):
        ...
        from kafka.producer import KafkaProducer
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = KafkaConfigurationDTO(enabled=True, bootstrap_servers='localhost:9092', dlq_topic='app.dlq')
        mock_cfg_cls.return_value = mock_cfg
        p = KafkaProducer()
        p._producer = MM()
        p._producer.send_and_wait = AsyncMock()
        env = KafkaJsonEnvelope(message_type='Err', payload={'n': 1})
        asyncio.run(p.send_json_envelope_to_dlq(env, original_topic='orders', error='boom', partition=2, offset=9))
        p._producer.send_and_wait.assert_awaited_once()
        ca = p._producer.send_and_wait.await_args
    ...
    def test_producer_send_json_envelope_to_dlq_requires_topic(self, mock_cfg_cls):
        ...
        from kafka.producer import KafkaProducer
        mock_cfg = MagicMock()
        mock_cfg.get_config.return_value = KafkaConfigurationDTO(enabled=True, bootstrap_servers='localhost:9092')
        mock_cfg_cls.return_value = mock_cfg
        p = KafkaProducer()
        p._cfg = mock_cfg.get_config.return_value
        with pytest.raises(ValueError, match='dlq_topic'):
            asyncio.run(p.send_json_envelope_to_dlq(KafkaJsonEnvelope(message_type='E', payload={}), original_topic='t', error='e'))
