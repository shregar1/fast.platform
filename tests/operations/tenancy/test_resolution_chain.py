...
from tenancy.resolution import ResolutionStrategy, ResolverSpec, TenantResolverRegistry, subdomain_then_header
...
class TestResolutionChain(ITenancyTests):
    @pytest.mark.asyncio
    async def test_subdomain_then_header_subdomain_wins(self, store_with_acme: InMemoryTenantStore) -> None:
    ...
    @pytest.mark.asyncio
    async def test_subdomain_then_header_fallback_header(self, store_with_acme: InMemoryTenantStore) -> None:
        resolver = subdomain_then_header(store_with_acme, base_domain="example.com")
