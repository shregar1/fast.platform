...
def test_fanout_request_push_only():
    req = NotificationFanoutRequest(
        title="Hi",
        body="Body",
        push=PushNotificationTarget(ios_device_tokens=["t1"]),
    )
    assert req.email is None


def test_fanout_request_both_channels():
    req = NotificationFanoutRequest(
        title="Hi",
        body="Body",
        email=EmailNotificationTarget(to=["a@example.com"]),
...
def test_fanout_rejects_no_channel():
    with pytest.raises(ValueError, match="At least one"):
        NotificationFanoutRequest(title="x", body="y")


def test_fanout_rejects_empty_push():
...
def test_retry_policy_dto_roundtrip():
