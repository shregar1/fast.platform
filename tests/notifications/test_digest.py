...
def test_digest_buffer_drain_all():
    buf = DigestBuffer()
...
def test_build_digest_fanout_request():
    req = build_digest_fanout_request(
        items=[
            DigestItem(title="One", body="a"),
            DigestItem(title="Two", body="b"),
        ],
        digest_title="Daily",
...
def test_build_digest_requires_channel():
...
def test_build_digest_push_only():
    req = build_digest_fanout_request(
        items=[DigestItem(title="x", body="y")],
