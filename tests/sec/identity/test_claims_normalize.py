...
class TestClaimsNormalize(IIdentityTests):
    ...
    def test_normalize_roles_keycloak_style(self):
        claims = {'realm_access': {'roles': ['admin', 'user']}, 'resource_access': {'client': {'roles': ['api']}}, 'roles': ['direct']}
    ...
    def test_normalize_token_claims_mfa(self):
        claims = {'sub': 'user-1', 'iss': 'https://idp/', 'aud': 'my-api', 'realm_access': {'roles': ['r1']}, 'scope': 'api.read', 'amr': ['pwd', 'otp'], 'acr': 'silver'}
