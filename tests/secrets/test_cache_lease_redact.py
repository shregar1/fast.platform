...
from fast_secrets.cache import CachedSecretsBackend
from fast_secrets.lease import LeasedSecretsBackend
from fast_secrets.redact import redact_json_for_log, redact_mapping, redact_text
...
def test_cached_ttl_and_force_refresh():
    inner = MagicMock()
...
def test_lease_triggers_background_refresh():
    b = _MemBackend()
    b.store["k"] = "first"
...
def test_redact_text_order():
    assert redact_text("hello supersecret and sub", "sub", "supersecret", mask="[REDACTED]") == (
