"""Tests for CachedSecretsBackend."""
...
def test_ttl_avoids_upstream_until_expiry():
    inner = MemoryBackend()
    inner.store["a"] = "1"
    clock = {"t": 0.0}
...
def test_rotation_callback_when_value_changes_after_refresh():
    inner = MemoryBackend()
    inner.store["k"] = "old"
...
def test_notify_on_first_fetch():
    inner = MemoryBackend()
    inner.store["k"] = "v"
    events: list[tuple[str, Optional[str], Optional[str]]] = []
    ...
    c = CachedSecretsBackend(
        inner,
        on_rotation=on_rot,
        notify_on_first_fetch=True,
    )
    c.get_secret("k")
    assert events == [("k", None, "v")]


def test_set_secret_triggers_rotation_when_cached_value_differs():
    inner = MemoryBackend()
    events: list[tuple[str, Optional[str], Optional[str]]] = []
