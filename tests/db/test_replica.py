...
def test_read_replica_url_none_when_not_configured():
...
def test_read_replica_url_formats():
    cfg = DBConfigurationDTO(
        user_name="u",
        password="p",
        host="primary",
...
def test_get_read_engine_none_without_replica():
...
def test_create_read_session_factory_none():
    assert create_read_session_factory(DBConfigurationDTO()) is None


def test_read_db_dependency_raises_when_no_session():
