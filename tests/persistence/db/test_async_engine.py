...
import pytest
from fast_platform.dtos import DBConfigurationDTO
...
class TestAsyncEngine(IDatabaseTests):
    ...
    def test_async_database_url_postgres_derives_asyncpg(self):
        config = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}')
    ...
    def test_async_database_url_explicit_override(self):
        config = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', async_connection_string='postgresql+asyncpg://{user_name}:{password}@{host}:{port}/{database}')
        url = async_database_url_from_config(config)
    ...
    def test_get_async_engine_uses_default_config(self, mock_db_cls):
        mock_conf = MagicMock()
        mock_conf.get_config.return_value = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}')
    ...
    def test_create_and_set_async_session_factory_none_config_uses_singleton(self, mock_get_eng, mock_create_fac):
        mock_conf = MagicMock()
        mock_conf.get_config.return_value = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}')
        mock_db = MagicMock()
    ...
    def test_get_async_engine_passes_pool_async(self, mock_create):
        config = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', pool_size=2, pool_recycle=100)
    ...
    def test_get_async_engine_passes_server_settings(self, mock_create):
        config = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', connection_name='async-primary', statement_timeout_seconds=3.0)
        get_async_engine(config)
        _, kwargs = mock_create.call_args
    ...
    def test_async_read_replica_url_from_config(self):
        ...
        cfg = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', read_replica_connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}')
    ...
    def test_get_async_read_engine_none(self):
        from db.async_engine import get_async_read_engine
        assert get_async_read_engine(DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}')) is None
    ...
    def test_async_database_url_sqlite_derived(self):
        config = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='app', connection_string='sqlite:///./{database}.db')
        url = async_database_url_from_config(config)
    ...
    def test_async_database_url_sqlite_two_slash_form(self):
        ...
        config = DBConfigurationDTO(user_name='u', password='p', host='localhost', port=5432, database='d', connection_string='sqlite://{host}/db.sqlite')
        url = async_database_url_from_config(config)
        assert 'sqlite+aiosqlite' in url

    def test_async_database_url_unknown_driver_raises(self):
        config = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='mysql+pymysql://{user_name}:{password}@{host}:{port}/{database}')
