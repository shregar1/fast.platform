...
class TestEnginePool(IDatabaseTests):
    ...
    def test_get_engine_passes_pool_kwargs(self):
        config = DBConfigurationDTO(user_name='u', password='p', host='localhost', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', pool_size=3, max_overflow=7, pool_timeout=15.0, pool_recycle=3600, pool_pre_ping=False)
    ...
    def test_pool_recycle_omitted_when_none(self):
        config = DBConfigurationDTO(user_name='u', password='p', host='localhost', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', pool_recycle=None)
        with patch('db.engine.create_engine') as mock_ce:
            get_engine(config)
            _, kwargs = mock_ce.call_args
    ...
    def test_get_engine_passes_postgres_connect_args(self, mock_ce):
        ...
        config = DBConfigurationDTO(user_name='u', password='p', host='localhost', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', connection_name='primary-api', statement_timeout_seconds=2.5)
        get_engine(config)
        mock_ce.assert_called_once()
        _, kwargs = mock_ce.call_args
