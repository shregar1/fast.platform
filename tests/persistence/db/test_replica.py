...
from db.replica import ReadDBDependency, create_and_set_read_session, create_read_session_factory, get_read_engine, read_replica_url_from_config

class TestReplica(IDatabaseTests):

    def test_read_replica_url_none_when_not_configured(self):
        cfg = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}')
        assert read_replica_url_from_config(cfg) is None

    def test_read_replica_url_formats(self):
        cfg = DBConfigurationDTO(user_name='u', password='p', host='primary', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', read_replica_connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', read_replica_host='replica.internal')
    ...
    def test_get_read_engine_passes_connect_args(self, mock_ce):
        cfg = DBConfigurationDTO(user_name='u', password='p', host='h', port=5432, database='d', connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', read_replica_connection_string='postgresql+psycopg2://{user_name}:{password}@{host}:{port}/{database}', connection_name='read-api', statement_timeout_seconds=5.0)
        get_read_engine(cfg)
