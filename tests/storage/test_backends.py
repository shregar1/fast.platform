...
def test_storage_factory_selects_local_backend(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(storage_base, "StorageConfiguration", lambda: types.SimpleNamespace(get_config=lambda: FakeCfg()), raising=True)
...
def test_build_storage_backend_import_error_returns_none(monkeypatch):
    ...
    class FakeCfg:
        def __init__(self):
            self.s3 = types.SimpleNamespace(enabled=True, bucket="b", region="us-east-1", endpoint_url=None, access_key_id=None, secret_access_key=None, base_path="")
            self.gcs = types.SimpleNamespace(enabled=True, bucket="g", credentials_json_path=None, base_path="")
            self.azure_blob = types.SimpleNamespace(enabled=True, container="c", connection_string=None, account_url="http://acc", base_path="")
...
def test_storage_factory_success_builds_s3_gcs_and_azure(monkeypatch, tmp_path):
    ...
    fake_boto3.client = lambda service_name, config=None, **kwargs: object()  # type: ignore[assignment]
    ...
    fake_storage_mod.Client = FakeStorageClient  # type: ignore[attr-defined]
    sys.modules["google"] = types.ModuleType("google")
    sys.modules["google.cloud"] = types.ModuleType("google.cloud")
    sys.modules["google.cloud"].storage = fake_storage_mod  # type: ignore[attr-defined]
    ...
    fake_azure_blob.BlobServiceClient = fake_blob_service_client  # type: ignore[attr-defined]
    fake_azure_identity = types.ModuleType("azure.identity")
    fake_azure_identity.DefaultAzureCredential = lambda: object()  # type: ignore[attr-defined]
    sys.modules["azure.storage.blob"] = fake_azure_blob
    sys.modules["azure.identity"] = fake_azure_identity

    class FakeCfg:
        def __init__(self):
            self.s3 = types.SimpleNamespace(enabled=True, bucket="b", region="us-east-1", endpoint_url=None, access_key_id=None, secret_access_key=None, base_path="base")
            self.gcs = types.SimpleNamespace(enabled=True, bucket="g", credentials_json_path=None, base_path="base")
            self.azure_blob = types.SimpleNamespace(enabled=True, container="c", connection_string="cs", account_url=None, base_path="base")
...
def test_s3_backend_upload_download_delete_and_presigned_url(monkeypatch):
    ...
    fake_boto3.client = _fake_client  # type: ignore[assignment]
    sys.modules["boto3"] = fake_boto3
    ...
    assert backend._client.put_calls  # type: ignore[attr-defined]
    assert backend._client.upload_fileobj_calls  # type: ignore[attr-defined]

    assert backend.download("k1") == b"payload"
    backend.delete("k1")
    assert backend._client.deleted[-1]["Key"] == "base/k1"  # type: ignore[attr-defined]
...
def test_gcs_backend_upload_download_delete_and_presigned_url(monkeypatch):
    ...
    backend = GCSStorageBackend(bucket="bucket", credentials_path="creds.json", base_path="base")
    url = backend.upload("k1", b"v1", content_type="text/plain")
    assert url == "gs://bucket/base/k1"

    # Cover file-like upload path.
    backend.upload("k2", io.BytesIO(b"v2"))
...
def test_azure_blob_backend_upload_download_delete(monkeypatch, tmp_path):
    ...
    fake_azure_blob = types.ModuleType("azure.storage.blob")
    fake_azure_blob.BlobServiceClient = FakeBlobServiceClient  # type: ignore[attr-defined]
    ...
    backend = AzureBlobStorageBackend(container="c", connection_string="cs", account_url=None, base_path="base")
    url = backend.upload("k1", b"v1", content_type="text/plain")
    assert url == f"https://blob/base/k1"
    assert backend.download("k1") == b"azure-bytes"
    backend.delete("k1")

    # Cover upload with file-like object (else branch).
    backend.upload("k2", io.BytesIO(b"v2"))
...
def test_azure_blob_backend_account_url_branch_and_missing_args(monkeypatch):
    ...
    backend = AzureBlobStorageBackend(container="c", connection_string=None, account_url="http://acc", base_path="")
    # Smoke-test key path building.
    backend.upload("k1", b"v1")
    backend.delete("k1")

    with pytest.raises(ValueError):
        AzureBlobStorageBackend(container="c", connection_string=None, account_url=None, base_path="")
...
def test_azure_blob_backend_dependency_missing_raises_runtime_error(monkeypatch):
    ...
    monkeypatch.setattr(builtins, "__import__", _deny_import)
    with pytest.raises(RuntimeError):
        AzureBlobStorageBackend(container="c", connection_string="cs", account_url=None, base_path="")
