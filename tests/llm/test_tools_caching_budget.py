...
def test_tool_definition_roundtrip_openai_anthropic() -> None:
    t = ToolDefinition(
        name="get_weather",
...
def test_normalize_mixed_tool_lists() -> None:
    openai_shape = {
        "type": "function",
        "function": {
            "name": "f",
...
def test_tool_definitions_lists() -> None:
    tools = [
...
def test_check_usage_against_budget() -> None:
...
@pytest.mark.asyncio
async def test_iter_llm_stream_with_budget_usage() -> None:
...
@pytest.mark.asyncio
async def test_iter_llm_stream_with_budget_raises_on_estimate() -> None:
    big = "x" * 400
