...
class TestHashingService(IServicesTests):
...
class TestKeyRotationService(IServicesTests):
...
class TestAesGcmCryptoService(IServicesTests):
    def test_roundtrip(self) -> None:
