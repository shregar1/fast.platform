...
def test_build_search_backend_meilisearch_and_methods(monkeypatch):
    ...
    class FakeMeiliClient:
        ...
        def delete_index(self, index_name: str) -> None:
            self.deleted = index_name  # type: ignore[attr-defined]

    # Inject fake provider module.
    fake_mod = types.ModuleType("meilisearch")
    fake_mod.Client = FakeMeiliClient  # type: ignore[attr-defined]
    _install_module("meilisearch", fake_mod)

    # Inject fake SearchConfiguration result.
    class FakeCfg:
        def __init__(self):
    ...
    # Cover MeilisearchBackend methods.
    docs = [{"a": 1}]
...
def test_opensearch_backend_index_search_delete_and_filter(monkeypatch):
    ...
    # Inject fake dependency module.
    fake_mod = types.ModuleType("opensearchpy")
    fake_mod.OpenSearch = FakeClient  # type: ignore[attr-defined]
    ...
    # Verify filter branch added an extra bool query (before search_faceted overwrites body).
    ...
    fr = backend.search_faceted(
        "products", "query", limit=3, offset=2, filter={"category": "books"}, facet_fields=["category"]
    )
    assert fr.total == 2
...
def test_typesense_backend_index_search_delete_and_filter(monkeypatch):
    ...
    fake_mod.Client = FakeTypesenseClient  # type: ignore[attr-defined]
    ...
    # Helper should cover via backend path, but assert separately for clarity.
