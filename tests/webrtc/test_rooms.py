...
def test_registry_idempotent_join_updates_display_name():
...
def test_registry_rejects_zero_capacity():
    with pytest.raises(ValueError):
        InMemoryRoomRegistry(max_participants_per_room=0)


def test_get_room_list_participants_leave_when_missing():
    reg = InMemoryRoomRegistry()
...
def test_rejoin_updates_metadata():
    reg = InMemoryRoomRegistry()
