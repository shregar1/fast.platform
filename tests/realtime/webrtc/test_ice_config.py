...
from fast_platform.dtos import RealtimeConfigurationDTO, WebRtcIceConfigDTO, WebRtcIceServerDTO
...
class TestIceConfig(IWebRTCTests):
    ...
    def test_rtc_ice_servers_when_enabled(self):
        cfg = RealtimeConfigurationDTO(webrtc=WebRtcIceConfigDTO(enabled=True, ice_servers=[WebRtcIceServerDTO(urls='stun:stun.l.google.com:19302'), WebRtcIceServerDTO(urls=['turn:turn.example.com:3478'], username='u', credential='p')]))
    ...
    def test_legacy_flat_urls(self):
        ...
        assert ice_servers_from_legacy_webrtc_dto(dto) == [{'urls': 'stun:a:1'}, {'urls': 'turn:b:2'}]
