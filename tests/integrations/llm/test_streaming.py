...
async def _mock_openai_stream() -> AsyncIterator[Any]:
    ch = types.SimpleNamespace(choices=[types.SimpleNamespace(delta=types.SimpleNamespace(content='hello'), finish_reason=None)])
    yield ch
    u = types.SimpleNamespace(choices=[], usage=types.SimpleNamespace(prompt_tokens=1, completion_tokens=2, total_tokens=3))
    yield u
