...
class TestCrud(IAdminTests):
    ...
    def test_crud_roundtrip_and_audit(self, session_dep):
        ...
        async def audit_hook(*, action: str, resource_type: str, resource_id: Optional[str], details: Optional[dict[str, Any]], request: Optional[Any]=None) -> None:
            events.append({'action': action, 'resource_type': resource_type, 'resource_id': resource_id})
        app = FastAPI()
        app.include_router(crud_router_from_model(Widget, create_schema=WidgetCreate, update_schema=WidgetUpdate, read_schema=WidgetRead, session_dep=session_dep, resource_type='widgets', audit=audit_hook), prefix='/widgets')
    ...
    def test_audit_repository_hook_and_actor(self, session_dep):
        ...
        class MemAudit(IAuditLogRepository):
            ...
            async def append(self, action: str, resource_type: str, *, actor_id: Optional[str]=None, resource_id: Optional[str]=None, details: Optional[dict[str, Any]]=None, ip_address: Optional[str]=None, user_agent: Optional[str]=None) -> AuditLogEntry:
                self.appends.append({'action': action, 'resource_type': resource_type, 'actor_id': actor_id, 'resource_id': resource_id})
                return AuditLogEntry(id='1', action=action, resource_type=resource_type, resource_id=resource_id, details=details or {}, created_at=datetime.now(timezone.utc))
        ...
        def actor(req: Request) -> Optional[str]:
            return 'actor-1'
        app = FastAPI()
        app.include_router(crud_router_from_model(Widget, create_schema=WidgetCreate, update_schema=WidgetUpdate, read_schema=WidgetRead, session_dep=session_dep, audit=repo, get_actor_id_from_request=actor), prefix='/w')
        client = TestClient(app)
    ...
    def test_audit_repository_hook_factory(self, session_dep):
        ...
        class MemAudit(IAuditLogRepository):
            ...
            async def append(self, action: str, resource_type: str, **kwargs: Any) -> AuditLogEntry:
                ...
                return AuditLogEntry(id='1', action=action, resource_type=resource_type, created_at=datetime.now(timezone.utc))
        ...
        app = FastAPI()
        app.include_router(crud_router_from_model(Widget, create_schema=WidgetCreate, update_schema=WidgetUpdate, read_schema=WidgetRead, session_dep=session_dep, audit=hook), prefix='/w')
        client = TestClient(app)
