"""Tests for QueueMessageEnvelope."""
...
def test_with_processing_failure_and_quarantine():
...
def test_should_quarantine_function():
...
def test_from_dict_invalid():
    with pytest.raises(ValueError):
        QueueMessageEnvelope.from_dict([])  # type: ignore[arg-type]
    with pytest.raises(ValueError):
