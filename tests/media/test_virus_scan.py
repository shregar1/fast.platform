...
def test_pipeline_rejects_when_scan_raises():
...
def test_clamscan_clean(monkeypatch):
    proc = MagicMock()
    proc.returncode = 0
    proc.stdout = b""
    proc.stderr = b""
...
def test_clamscan_infected(monkeypatch):
    proc = MagicMock()
    proc.returncode = 1
...
def test_clamd_found(monkeypatch):
