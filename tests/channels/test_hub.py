...
def test_hub_connect_broadcast_and_disconnect_cleanup():
    ...
    # Manual disconnect of remaining websocket should delete topic when empty.
...
def test_hub_broadcast_disconnects_broken_socket():
    ...
    # Broadcast should drop the failing websocket and keep others.
...
def test_hub_send_ping_record_pong_and_sweep():
    ...
    # Fresh connection: pong updates last_pong_at so sweep does not drop.
    ws2 = FakeWebSocket()
    asyncio.run(hub.connect("room1", ws2))
