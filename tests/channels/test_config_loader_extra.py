...
def test_channels_config_uses_explicit_env_path():
    ...
    with patch("fast_channels.config_loader.open", mock_open(read_data=json.dumps(payload))):
        with patch("fast_channels.config_loader.os.getenv", side_effect=lambda k: env_path if k == "FASTMVC_CHANNELS_CONFIG_PATH" else None):
...
def test_channels_config_json_decode_error_results_in_empty_config():
    ...
    ChannelsConfiguration._instance = None
    with patch("fast_channels.config_loader.open", mock_open(read_data="not-json")):
        with patch("fast_channels.config_loader.json.load", side_effect=json.JSONDecodeError("bad", "doc", 0)):
